...
public class MarkController : Monster
{
    ...
    {
        base.Update();
        //this.gameObject.transform.position = new Vector2(speed, Mathf.Sin(Time.time * 2) * 1.5f);
        //transform.position = new Vector3(this.transform.position.x, Mathf.Sin(Time.time * 2) * 1.5f);
        //this.gameObject.transform.position = GetSWalkPos();

        /*
        a += 0.01f;//弧度每帧增加0.01f
        this.transform.Translate(Vector2.right * Time.deltaTime * 1.0f);//物体朝着自己的z轴正方向移动，移动速度为speed*deltatime
        this.transform.Translate(Vector2.up * Mathf.Sin(a) * 0.05f);//物体y轴方向移动是sin曲线形式（mathf.sin(a)表示将a这个弧度传入并转化为对应数值）（后面*0.1f是为了减弱速度）
        */

        //this.gameObject.transform.position += new Vector3(movePos.position.x, movePos.position.y, 0);
    ...
    Vector3 GetSWalkPos2()
    {
        ...
        Vector3 sWalkPos = new Vector3(x * 3, 0, z * 0.1f);
        return sWalkPos;
    }
